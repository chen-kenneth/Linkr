...
const ExpandableListItem = ({ item }) => {
  ...
  return (
    <View className="my-2 py-2 border-t border-gray-100 rounded-lg bg-primary">
    ...
    <View>
      {toggleOn ? (
        <View
          ...
          className="overflow-hidden bg-primary border-t border-gray-100"
        >
          {/* Grey pill with dragging functionality */}
          <DraggablePill dragY={dragY} {...panResponder.panHandlers} />

          <FlatList
            data={data.slice(0, showAll ? 6 : maxVisibleItems)} // Dynamically show more items or up to 6 when Show All is active
          ...
          <Button
            ...
            onPress={() => setShowAll(showAll)} // Toggle between Show All and Show Less
          />
        </View>
      ) : (
